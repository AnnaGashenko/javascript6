...
const setDescriptors = (obj, properies) => {
    for (const item of Object.entries(properies)) {
        const key = item[0];
        const value = item[1];
...
const shallowMerge = function (user, userData) {
    ...
    const properiesUser = Object.getOwnPropertyDescriptors(user);
    const properiesUserData = Object.getOwnPropertyDescriptors(userData);
    const properiesClone = { ...properiesUser, ...properiesUserData };

    const newObj = setDescriptors(userClone, properiesClone);
